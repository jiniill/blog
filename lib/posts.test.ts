...
describe("lib/posts 인덱스 유틸", () => {
  it("공개된 글만 반환한다", () => {
    expect(getPublishedPosts(defaultLocale).every((post) => post.published)).toBe(true);
  });

  it("공개 글 목록을 최신 날짜 순으로 정렬한다", () => {
    const sortedPosts = getSortedPublishedPosts(defaultLocale);
  ...
  it("태그 카운트와 태그별 글 목록 수가 일치한다", () => {
    getPublishedTagCounts(defaultLocale).forEach(([tag, count]) => {
      expect(getPublishedPostsByTag(defaultLocale, tag).length).toBe(count);
    });
  });
  ...
  it("인접 글 정보가 정렬 결과와 동일한 순서를 따른다", () => {
    ...
    const adjacentPosts = getAdjacentPublishedPosts(defaultLocale, currentPost.slug);
