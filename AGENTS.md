...
* In local development, run automated tests inside Docker (for example: `docker compose exec -T app php artisan test`); avoid running `php artisan test` directly on the host unless explicitly required.
